...
my $pwntter_host     = "localhost";
my $pwntter_db	     = "superpwntter";
my $pwntter_user_id  = "omniumie";
my $pwntter_password = "niska8821";
my $pwntter_version = "0.3";
my $pwntter_db_engine =  "Pg";
...
our $dbh = DBI->connect("DBI:$pwntter_db_engine:database=$pwntter_db;host=$pwntter_host", "$pwntter_user_id", "$pwntter_password", 
	{'RaiseError' => 0, 'my_pid' => $$});
...
# 	if ($user_created_at ne ''){ $user_created_at  = date_format($user_created_at);}
	...
	$fields_user->{'favourites_count'}                    = $ref->{'user'}->{'favourites_count'};
...
# 	&$pwntter_perform_query( \%fields_tweet , "tweets");
	...
	# Media
	if (defined $ref->{'entities'}->{'media'})
	{
		while (my ($i, $val) = each $ref->{'entities'}->{'media'}) {
	# 		print "$i => " . Dumper($val);
			my %fields_media = ();
	...
	# Mentions
	if (defined $ref->{'entities'}->{'user_mentions'})
	{
...
# 	print "$url -- $name -- $source \n";
	...
	if (true)	# Damned TTYtter multi-thread environs
	{
		$dbh = DBI->connect("DBI:$pwntter_db_engine:database=$pwntter_db;host=$pwntter_host", "$pwntter_user_id", "$pwntter_password", 
